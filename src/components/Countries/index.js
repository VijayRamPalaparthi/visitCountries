      ...
      <div className="bg-container">
        ...
        <ul className="list-container">
          {countriesList.map(each => {
            const onClickVisit = () => {
              this.onChangeToVisited(each.id)
            }
            return (
              <li className="country-list" key={each.id}>
        ...
        {visitedList.length === 0 ? (
          ...
          <ul className="visited-list-container">
            {visitedList.map(each => {
              ...
              return (
                <li className="visited-country-list" key={each.id}>
                  <img src={each.imageUrl} className="image" alt="thumbnail" />
                  <div className="br-container">
                    <p className="br-country-name">{each.name}</p>
                    <button
